# RK3588 快速部署脚�?- 使用本地预构建镜�?# 无需工控机联网，所有依赖在本地Windows机器下载并构�?
$ErrorActionPreference = "Stop"

Write-Host "=== RK3588 快速部署方�?===`n" -ForegroundColor Green
...
# 目标设备配置（可修改�?$TargetDevice\ = "155"  # 可�? "155" �?"170"
...
# 创建完整应用镜像的Dockerfile（基于预构建基础镜像�?$DockerfileContent = @"
...
# 复制系统�?COPY lib /app/libs

# 复制Qt6运行�?COPY qt6/lib /opt/qt6/lib
...
# 创建配置和数据目�?RUN mkdir -p /app/config /app/data
...
# 复制必要文件到构建目�?Write-Host "准备构建上下�?.." -ForegroundColor Gray
...
# 构建完整应用镜像
Write-Host "开始构建完整应用镜�?.." -ForegroundColor Gray
...
Write-Host "�?镜像已导�?($ImageSizeMB MB)" -ForegroundColor Green
Write-Host ""

# ===== 步骤 3: 传输到目标设�?=====
Write-Host "步骤 3/4: 传输镜像�?$DeviceIP..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "4. 分发到其他设�?"
Write-Host "   �?55上运�? scp ${DeviceUser}@${DeviceIP}:/tmp/belt-control.tar ."
