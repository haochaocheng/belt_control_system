#include "filter.h"
#include <QtQml>
#include <QRegularExpression> // Qt6: For RegExpFilter
...
void RegExpFilter::setPattern(const QString& pattern)
{
    ...
    m_pattern = pattern;
    // Qt6: No need to set m_regExp anymore, pattern is stored directly
    Q_EMIT filterChanged();
...
void RegExpFilter::setSyntax(QQmlSortFilterProxyModel::PatternSyntax syntax)
{
    ...
    m_syntax = syntax;
    // Qt6: Pattern syntax is stored directly, no QRegExp to update
    Q_EMIT filterChanged();
...
void RegExpFilter::setCaseSensitivity(Qt::CaseSensitivity caseSensitivity)
{
    ...
    // Qt6: Case sensitivity is stored directly, no QRegExp to update
    Q_EMIT filterChanged();
...
bool RegExpFilter::filterRow(const QModelIndex& sourceIndex) const
{
    ...
    // Qt6: Use QRegularExpression instead of QRegExp
    QRegularExpression regExp(m_pattern);
    if (m_caseSensitivity == Qt::CaseInsensitive) {
        regExp.setPatternOptions(QRegularExpression::CaseInsensitiveOption);
    }
    return regExp.match(string).hasMatch();
}
...
bool RangeFilter::filterRow(const QModelIndex& sourceIndex) const
{
    ...
    // Qt6: Use QVariant::compare() instead of direct comparison operators
    bool lessThanMin = m_minimumValue.isValid() &&
            (m_minimumInclusive ? QVariant::compare(value, m_minimumValue) == QPartialOrdering::Less
                                : (QVariant::compare(value, m_minimumValue) == QPartialOrdering::Less ||
                                   QVariant::compare(value, m_minimumValue) == QPartialOrdering::Equivalent));
    bool moreThanMax = m_maximumValue.isValid() &&
            (m_maximumInclusive ? QVariant::compare(value, m_maximumValue) == QPartialOrdering::Greater
                                : (QVariant::compare(value, m_maximumValue) == QPartialOrdering::Greater ||
                                   QVariant::compare(value, m_maximumValue) == QPartialOrdering::Equivalent));
...
// Qt6: Return type changed from int to qsizetype
...
// Qt6: Index parameter changed from int to qsizetype
