...
{
    // Qt6: use filterRegularExpression() instead of filterRegExp()
    return filterRegularExpression().pattern();
}

void QQmlSortFilterProxyModel::setFilterPattern(const QString& filterPattern)
{
    // Qt6: use filterRegularExpression() and setFilterRegularExpression()
...
{
    // Qt6: QRegularExpression doesn't have PatternSyntax like QRegExp did
    // We'll return RegExp as default since QRegularExpression always uses Perl-compatible syntax
    return static_cast<PatternSyntax>(RegExp);
}

void QQmlSortFilterProxyModel::setFilterPatternSyntax(QQmlSortFilterProxyModel::PatternSyntax patternSyntax)
{
    // Qt6: QRegularExpression doesn't support different pattern syntaxes like QRegExp did
    // QRegularExpression always uses Perl-compatible regular expressions
    // We'll just emit the signal for API compatibility
