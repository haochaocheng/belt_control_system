...
public:
    ...
    QList<QAbstractItemModel *> m_callHistoryModelsList; // Qt6: For QQmlListProperty
};
...
{
    static RisipCallManager *m_callManagerInstance = nullptr;
    if(m_callManagerInstance == nullptr)
...
{
    // Qt6: Update the list from the hash and return pointer to the member variable
    m_data->m_callHistoryModelsList = m_data->m_callHistoryModels.values();
    return QQmlListProperty<QAbstractItemModel>(this, &m_data->m_callHistoryModelsList);
}
...
{
    // Qt6: m_callManagerInstance is now a function-local static (Meyer's Singleton)
