...
Rectangle {
    ...
    ScrollView {
        ...
        GridLayout {
            ...
            columnSpacing: 10
            rowSpacing: 12
            // ❌ 2026-02-03 [FIX 100.300.112.8.25.7.12.4]: 移除无效的Layout.minimumHeight
            // 原因：Layout属性只在父布局是Layout时有效，在ScrollView中无效
            // 新方案：直接使用Flickable.contentY滚动，不依赖contentHeight
            ...
            Item {
                ...
                MouseArea {
                    anchors.fill: parent
                    onClicked: function(mouse) {
                        console.log("✅ [CurrentProtectionTab] 鼠标点击动作保护类型，发射信号: requestFocusParamIndex(4)")
            ...
            // ✅ 2026-02-02 [FIX 100.300.112.8.25]: 改为 CustomComboBox，选项 AI0.0-AI1.7
            // 输入点选择输入（下拉框）
            Item {
                Layout.column: 3
                Layout.row: 4
            ...
            // ✅ 2026-02-02 [FIX 100.300.112.8.25]: 改为 CustomSpinBox，修改单位为 "0.1秒"
            // 过滤干扰延时输入（可输入，带单位）
            Item {
                Layout.column: 1
                Layout.row: 5
