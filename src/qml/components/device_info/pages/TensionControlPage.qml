    ...
    DeviceInfo.NavigationManager {
        ...
        function moveInParamArea(direction) {
            ...
            if (root.currentControlIndex === 0) {
                // 张力传感器: 15个参数（4列布局，每行2个参数）
                // 2026-03-17 [Phase 7.48.52]: 参照AnalogInputPage重排后的导航数组
                // Row0: 名称(0)+播放次数(1), Row1: 模块类型(2)+播放时长(3),
                // Row2: 通道号(4)+上限值(5), Row3: 下限值(6)+量程(7),
                // Row4: 单位(8)+输入类型(9), Row5: 保护延时(10)+保护级别(11),
                // Row6: TTS文本(12)+音频文件(13), Row7: 洒水编号(14)
                rows = [[0,2],[2,2],[4,2],[6,2],[8,2],[10,2],[12,2],[14,1]]
            } else {
                // 旧：张紧控制: 6个参数  rows = [[0,1],[1,1],[2,2],[4,1],[5,1]]
                // ✅ 2026-03-22 [Phase 7.48.74]: 新增停止延时(4)，语音字段后移(5,6)，共7个参数
                // Row0: 输出通道(0), Row1: 反馈通道(1),
                // Row2: 反馈超时(2)+启动延时(3), Row3: 停止延时(4),
                // Row4: 预警语音(5), Row5: 失败语音(6)
                rows = [[0,1],[1,1],[2,2],[4,1],[5,1],[6,1]]
            }
