...
// �?2026-01-25 [Y轴振动保护] Y轴振动保护配�?// �?2026-01-28 [FIX 100.300.83] 替换所有只读显示框�?CustomReadOnlyField
...
Rectangle {
    ...
    function triggerParamInput(paramIndex) {
        ...
        case 3:  // 故障保护类型（只读）
            ...
            break
        case 4:  // 温度量程（只读）
            console.log("�?[YAxisVibrationTab] 温度量程（只读）")
            break
        case 5:  // 温度上限（只读）
            console.log("�?[YAxisVibrationTab] 温度上限（只读）")
            break
        case 6:  // 温度下限（只读）
            console.log("�?[YAxisVibrationTab] 温度下限（只读）")
            break
        case 7:  // 过滤干扰延时（只读）
