#!/bin/bash
# ============================================================================
# FFmpeg Compilation Script for Windows (MSYS2)
# ============================================================================
# This script compiles FFmpeg 6.0 with x264 support for Windows using MSYS2
#
# Requirements:
#   - MSYS2 with MinGW-w64 toolchain
#   - x264 already compiled (run build_x264_windows.sh first)
#   - FFmpeg 6.0 source code
#   - yasm or nasm installed
#
# Output:
...
if [ -f "${FFMPEG_INSTALL}/lib/libavcodec.dll.a" ]; then
    ...
    # Test H.264 codec availability
    echo "Checking H.264 codec availability..."
    if ${FFMPEG_INSTALL}/bin/ffmpeg.exe -codecs 2>/dev/null | grep -i h264 &>/dev/null; then
        echo "[OK] H.264 codec found"
    else
        echo "[WARNING] H.264 codec may not be available"
    fi
