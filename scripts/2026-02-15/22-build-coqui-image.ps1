...
<#
.SYNOPSIS
    构建Coqui TTS Docker镜像（缓存方案）
.DESCRIPTION
    构建包含Coqui TTS和所有依赖的Docker镜像
    一次构建，多次使用，避免重复安装
.EXAMPLE
    .\22-build-coqui-image.ps1
.NOTES
    2026-02-15 16:50: 创建 - 构建Coqui TTS缓存镜像
#>
...
Write-Host "  构建Coqui TTS Docker镜像" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host ""
Write-Host "  🐳 开始构建镜像（这可能需要15-20分钟）..." -ForegroundColor Cyan
Write-Host "  💡 主要时间用于下载PyTorch和Coqui TTS（约600 MB）" -ForegroundColor Gray
Write-Host ""
...
Write-Host "🚀 下一步: 使用镜像下载Coqui TTS模型" -ForegroundColor Yellow
Write-Host "   .\scripts\2026-02-15\23-download-coqui-with-cache.ps1" -ForegroundColor Gray
Write-Host ""
